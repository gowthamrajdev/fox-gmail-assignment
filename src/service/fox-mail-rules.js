    ...
    if (PREDICATE_DATE[rule.predicate] === PREDICATE_DATE.LESS_THAN) {
        return {
            [FIELD[rule.field]]: {
              ...
              [PREDICATE_DATE.GREATER_THAN]: moment(now).subtract(rule.value, DATE_INTERVAL_TYPE.DAY).format('YYYY-MM-DD HH:MM:SS')
            }
          }
    } else {
        return {
            [FIELD[rule.field]]: {
              [PREDICATE_DATE.LESS_THAN]: moment(now).add(rule.value, DATE_INTERVAL_TYPE.DAY).format('YYYY-MM-DD HH:MM:SS'),
